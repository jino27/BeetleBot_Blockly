...
export function initBeetleBotGenerator(): void {
  ...
  javascriptGenerator.forBlock[BLOCK_TYPES.TURN_LEFT] = () => `queue.push({cmd:"L"});\n`;
  javascriptGenerator.forBlock[BLOCK_TYPES.TURN_RIGHT] = () => `queue.push({cmd:"R"});\n`;
...
export function generateCommandQueue(workspace: Blockly.Workspace): CommandItem[] {
  ...
  const fullCode = `var queue=[];var currentSpeed=150;${code}return queue;`;
